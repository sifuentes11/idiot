    ...
    (do
      ...
      (doseq [n filesArray]
        (if (= (subs (.getName n) 0 addressLength) last38)
          (if (= fullAddress2 "")
            (def fullAddress2 (str first2 (.getName n)))
            (do
      ...
      (if (= fullAddress2 "")
        (do
          ...
          (System/exit 0))
        (str fullAddress2)))
    (str address)))       ; should this be an error message instead?

(defn getFullAddressWeb [address]
  (if (< (count address) 4)
    (str "")
    (do
      (def fullAddress2 "")
...
(defn ambiguous [address]
  (if (< (count address) 4)
    ...
    (do
      ...
      (def addressLength (count last38))
      ;(def filesArray (.listFiles (clojure.java.io/file (str workingDirectory idiotDirectory "refs/heads/"))))
      (def wholeBody "<p>The given address prefix is ambiguous. Please disambiguate your intent by choosing from the following options.</p><ul class=\"disambiguation-list\">")
      (if (.exists (clojure.java.io/as-file (str workingDirectory idiotDirectory "objects/" first2)))
        (do
          (def filesArray (.listFiles (clojure.java.io/file (str workingDirectory idiotDirectory "objects/" first2))))
          (doseq [n filesArray]
            (def wholeBody (str wholeBody "<li><a href=\"/" (getObjectTypeFromAddressString (str first2 (.getName n))) "/" (str first2 (.getName n)) "\">" (str first2 (.getName n)) "</a> (" (getObjectTypeFromAddressString (str first2 (.getName n))) ")</li>")))

          {:status 300  ; meaning "OK"
           :headers {"Content-Type" "text/html"}  ; instead of e.g. "text/plain"
           :body (str "<!DOCTYPE html><html><head><title>Yo yo yo</title></head><body>" wholeBody "</ul></body></html>")})
        (str "")))))
...
(defn rootStr []
  ...
  (def htmlBody "")
  (doseq [n newf]
    (def htmlBody (str htmlBody "<li><a href =\"/branch/" (.getName n) "\">" (.getName n) "</a></li>")))
  (str "<div class=\"head-info\">HEAD points to ref <a href=\"/branch/" (clojure.string/trim (subs (slurp (str workingDirectory idiotDirectory "HEAD")) 16 (index-of (slurp (str workingDirectory idiotDirectory "HEAD")) "\n"))) "\">" (clojure.string/trim (subs (slurp (str workingDirectory idiotDirectory "HEAD")) 16 (index-of (slurp (str workingDirectory idiotDirectory "HEAD")) "\n"))) "</a></div><ul class =\"branch-list\">" htmlBody "</ul>"))

(defn branchStr [refName1]

  (if (not (.exists (clojure.java.io/as-file (str workingDirectory idiotDirectory "refs/heads/" refName1))))
    (do
      {:status 404  ; meaning "OK"
       :headers {"Content-Type" "text/html"}})
    (do

      (def branchBody "")
      (def numberOfParents 1000000)
      (def refName (slurp (str workingDirectory idiotDirectory "refs/heads/" refName1)))
      ...
      (while (and (not (= refName "noMore")) (< counter numberOfParents))
        (do
          ...
          (def branchBody (str branchBody (str "<li><a href=\"/commit/" (subs refName 0 7) "\">" (subs refName 0 7) "</a> " commitMessageLine1 "</li>")))
      ...
      {:status 200  ; meaning "OK"
       :headers {"Content-Type" "text/html"}  ; instead of e.g. "text/plain"
       :body (str "<!DOCTYPE html><html><head><title>Yo yo yo</title></head><body>" (str "<ul class=\"commit-list\">" branchBody "</ul>") "</body></html>")})))

(defn commitStr [commitAddr1]

  (if (< (count commitAddr1) 40)
    (do
      (def fullAddress (getFullAddressWeb commitAddr1)))
    (do
      (def fullAddress (getFullAddressWeb commitAddr1))))
  ...
  (if (= "" fullAddress)
    (do
      {:status 404  ; meaning "OK"
       :headers {"Content-Type" "text/html"}})
    (if (< (count fullAddress) 40)
      (ambiguous fullAddress)
      (do
        ...
        (if (not (.exists (clojure.java.io/as-file (str workingDirectory idiotDirectory "objects/" myDir "/" myFName))))
          (do
            {:status 404  ; meaning "OK"
             :headers {"Content-Type" "text/html"}})
          (do
            (if (= (getObjectTypeFromAddressString fullAddress) "tree")
              (do
                {:status 302  ; meaning "OK"
                 :headers {"Location" (str "/tree/" commitAddr1)}})
              (if (= (getObjectTypeFromAddressString fullAddress) "blob")
                (do
                  {:status 302  ; meaning "OK"
                   :headers {"Location" (str "/blob/" commitAddr1)}})
                (if (= (getObjectTypeFromAddressString fullAddress) "commit")
                  (do
                    ...
                    (def wholeBody (str "<h1>Commit " commitAddr1 "</h1>" "<div class=\"tree\">tree <a href=\"/tree/" (subs blobbyStr2 5 45) "\">" (subs blobbyStr2 5 45) "</a></div>"))

                    (def potParInd 46)
                    (while (= (subs blobbyStr2 potParInd (+ potParInd 6)) "parent")
                      (def wholeBody (str wholeBody "<div class=\"parent\">parent <a href=\"/commit/" (subs blobbyStr2 (+ potParInd 7) (+ potParInd 47)) "\">" (subs blobbyStr2 (+ potParInd 7) (+ potParInd 47)) "</a></div>"))
                      (def potParInd (+ potParInd 48)))
                    (def wholeBody (str wholeBody "<div class=\"author\">" (subs blobbyStr2 (index-of blobbyStr2 "author") (+ (index-of blobbyStr2 "author") 21)) " &lt;" (subs blobbyStr2 (+ (index-of blobbyStr2 "<") 1) (+ (index-of blobbyStr2 ">") 0)) "&gt;" (subs blobbyStr2 (+ (index-of blobbyStr2 ">") 1) (+ (index-of blobbyStr2 ">") 18)) "</div>"))
                    (def wholeBody (str wholeBody "<div class=\"committer\">committer Linus Torvalds &lt;torvalds@transmeta.com&gt; 1581997446 -0500</div>"))
                    (def wholeBody (str wholeBody "<pre class=\"message\">" (subs blobbyStr2 (+ (index-of blobbyStr2 "\n\n") 2)) "</pre>"))
                    {:status 200  ; meaning "OK"
                     :headers {"Content-Type" "text/html"}  ; instead of e.g. "text/plain"
                     :body (str "<!DOCTYPE html><html><head><title>Yo yo yo</title></head><body>" wholeBody "</body></html>")})
                  ())))))))))

(defn treeStr [treeAddr1]
  (if (< (count treeAddr1) 40)
    (do
      (def fullAddress (getFullAddressWeb treeAddr1)))
    (do
      (def fullAddress (getFullAddressWeb treeAddr1))))
        ...
        (if (not (.exists (clojure.java.io/as-file (str workingDirectory idiotDirectory "objects/" myDir "/" myFName))))
          ...
          (do
            (if (= (getObjectTypeFromAddressString fullAddress) "commit")
              (do
                {:status 302  ; meaning "OK"
                 :headers {"Location" (str "/commit/" treeAddr1)}})
              (if (= (getObjectTypeFromAddressString fullAddress) "blob")
                (do
                  {:status 302  ; meaning "OK"
                   :headers {"Location" (str "/blob/" treeAddr1)}})
                (if (= (getObjectTypeFromAddressString fullAddress) "tree")
                  (do
                    (def blobby (unzip-binary (str workingDirectory idiotDirectory "objects/" myDir "/" myFName)))
                    ;(def blobbyStr2 (bytes->str (nth (split-at-byte 0 blobby) 0)))

                    (def wholeBody (str "<h1>Tree " treeAddr1 "</h1>" "<ul class=\"tree-entries\">"))
                    ...
                    (def wholeBody (str wholeBody "<li><tt>" treeMode " " (getObjectTypeFromAddressString treeAddress) " <a href=/" (getObjectTypeFromAddressString treeAddress) "/" treeAddress ">" treeAddress "</a> " treeType "</tt></li>"))
                    ...
                    (def wholeBody (str wholeBody "</ul>"))
...
(defn blobStr [blobAddr1]

  (if (< (count blobAddr1) 40)
    (do
      (def fullAddress (getFullAddressWeb blobAddr1)))
    (do
      (def fullAddress (getFullAddressWeb blobAddr1))))
        ...
        (if (not (.exists (clojure.java.io/as-file (str workingDirectory idiotDirectory "objects/" myDir "/" myFName))))
          ...
          (do
            (if (= (getObjectTypeFromAddressString fullAddress) "commit")
              (do
                {:status 302  ; meaning "OK"
                 :headers {"Location" (str "/commit/" blobAddr1)}})
              (if (= (getObjectTypeFromAddressString fullAddress) "tree")
                (do
                  {:status 302  ; meaning "OK"
                   :headers {"Location" (str "/tree/" blobAddr1)}})
                (if (= (getObjectTypeFromAddressString fullAddress) "blob")
                  (do
                    (def blobby (unzip-binary (str workingDirectory idiotDirectory "objects/" myDir "/" myFName)))
                    (def blobbyStr2 (bytes->str (nth (split-at-byte 0 blobby) 1)))
                    (def wholeBody (str "<h1>Blob " blobAddr1 "</h1>" "<pre>" blobbyStr2 "</pre>"))
...
(defroutes app
  (GET "/" []
    {:status 200  ; meaning "OK"
     :headers {"Content-Type" "text/html"}  ; instead of e.g. "text/plain"
     :body (str "<!DOCTYPE html><html><head><title>Yo yo yo</title></head><body>" (rootStr) "</body></html>")})
  (GET "/branch/:branchName" [branchName] (branchStr branchName))
  (GET "/commit/:commitAddr" [commitAddr] (commitStr commitAddr))
  (GET "/tree/:treeAddr" [treeAddr] (treeStr treeAddr))
  (GET "/blob/:blobAddr" [blobAddr] (blobStr blobAddr))
