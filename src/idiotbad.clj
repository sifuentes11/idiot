...
(defn getObjectTypeFromAddressString [oAddress]
  (def ^:dynamic oDir (subs oAddress 0 2))
  (def ^:dynamic oFile (subs oAddress 2))
  (def ^:dynamic fullObject (unzip-binary (str workingDirectory idiotDirectory "objects/" oDir "/" oFile)))
  (def ^:dynamic oHeader (bytes->str (nth (split-at-byte 0 fullObject) 0)))
  (subs oHeader 0 (index-of oHeader " ")))

(defn makeBlob [blobData]
  (def ^:dynamic myBlob (str "blob " (count blobData) "\000" blobData))
  (def ^:dynamic mySha1 (sha-bytes (.getBytes myBlob)))
  (def ^:dynamic mySha1Hex (to-hex-string mySha1))
  (def ^:dynamic myDir (subs mySha1Hex 0 2))
  (.mkdir (java.io.File. (str workingDirectory idiotDirectory "objects/" myDir)))
  (def ^:dynamic myFName (subs mySha1Hex 2))
...
(defn hash-object []
  ...
  (if (and (>= (count argList) 3) (= (nth argList 1) "-w"))
    (do
      (try
        ...
        (def ^:dynamic myBlobData (slurp (str workingDirectory (nth argList 2))))
        (catch Exception e
  ...
  (if (and (>= (count argList) 2) (not (= (nth argList 1) "-w")))
    (do
      (try
        (do
          (def ^:dynamic myBlobData (slurp (str workingDirectory (nth argList 1)))))
        (catch Exception e
          (println "Error: that file isn't readable")
          (System/exit 0)))
...
(defn cat-file []
  ...
  (def ^:dynamic myDir (subs (nth argList 2) 0 2))
  (def ^:dynamic myFName (subs (nth argList 2) 2))
  ...
  (def ^:dynamic blobby (unzip-binary (str workingDirectory idiotDirectory "objects/" myDir "/" myFName)))
  (def ^:dynamic blobbyStr (bytes->str (nth (split-at-byte 0 blobby) 0)))
  (def ^:dynamic objType (subs blobbyStr 0 (index-of blobbyStr " ")))
  ...
  (if (= (nth argList 1) "-p")
    (do
      (if (= objType "commit")
        (do
          (def ^:dynamic blobbyStr2 (bytes->str (nth (split-at-byte 0 blobby) 1)))
          (print blobbyStr2)
          (flush))
        ())
      (if (= objType "blob")
      ...
      (if (= objType "tree")
        (do
          (def ^:dynamic treeInfo (split-at-byte 0 (nth (split-at-byte 0 blobby) 1)))
          (def ^:dynamic treeHeader2 (bytes->str (nth treeInfo 0)))
          (def ^:dynamic treeMode (subs treeHeader2 0 (index-of treeHeader2 " ")))
          (def ^:dynamic treeType (subs treeHeader2 (+ (index-of treeHeader2 " ") 1)))
          (def ^:dynamic treeAddress (to-hex-string (nth treeInfo 1)))

          (if (= treeMode "40000")
            (def ^:dynamic treeMode (str "0" treeMode))
            ())
...
(defn branch []
  ...
  (if (and (> (count argList) 1) (= (nth argList 1) "-d"))
    ...
    ())
  (def ^:dynamic filesArray (.listFiles (clojure.java.io/file (str workingDirectory idiotDirectory "refs/heads/"))))
  (def ^:dynamic newf (sort filesArray))
  (doseq [n newf]
...
(defn write-wtree [curDir]
  ...
  (def ^:dynamic filesArray (.listFiles (clojure.java.io/file curDir)))
  ...
  (def ^:dynamic namesSorted (sort filesArray))
  (def myTreeContents "")                                      ;
